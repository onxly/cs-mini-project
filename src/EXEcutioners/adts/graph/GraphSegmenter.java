...
public class GraphSegmenter {
    ...
    private int[][] labels; // Maps every pixel (x,y) to a Superpixel ID
    private double[][] distances; // Stores the shortest distance found for each pixel
    ...
    public List<GraphNode> segment(int[][] pixelData, int regNum, double dblComp) {
        ...
        // Run for 5-10 iterations to let the "blobs" settle
        for (int i = 0; i < 10; i++) {
            ...
            for (GraphNode seed : seeds) {
                // Only look at a local 2S x 2S area around the seed
    ...
    private List<GraphNode> initializeSeeds(int[][] pixelData, int S) {
        ...
        // Iterate through the image in steps of S
        for (int x = S / 2; x < width; x += S) {
            for (int y = S / 2; y < height; y += S) {
                
                // Refinement: Find the best local position (lowest gradient)
